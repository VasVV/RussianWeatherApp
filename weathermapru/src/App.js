...
function App() {
  ...
  const weatherRequest = async (city, lat, lon) => {
    ...
    try {
      ...
      setWeather((prevState) => [
        ...prevState,
        { city, curr, tomorrow, opened: false },
      ]);
    } catch (err) {
      console.log(err);
    }
  ...
  const getWeather = () =>
    CITIES.forEach((e) =>
      weatherRequest(e.name, e.coordinates[1], e.coordinates[0])
    );

  const handleClick = (name) => {
    let newArr = [...weather];
    newArr.map((e) =>
      name === e.city ? (e.opened = true) : (e.opened = false)
    );
    setClicked(true);
    setWeather(newArr);
  };
                          ...
                          <g>
                            {population > 1000000 ? (
                              <text
                                ...
                                fill="red"
                                className={
                                  showOnZoom ? "text-show" : "text-hide"
                                }
                              >
                                <tspan x="1">
                                  Температура сейчас:{" "}
                                  {showOnZoom
                                    ? weather.find((e) => e.city === name).curr
                                        .temp
                                    : ""}
                                </tspan>
                                <tspan x="1" y="0">
                            ...
                            <text
                              ...
                              className={
                                clicked &&
                                weather.find((e) => e.city === name).opened
                                  ? "text-show"
                                  : "text-hide"
                              }
                            >
                              ...
                              <tspan x="1" y={6 / position.k}>
                                {" "}
                                Температура завтра:{" "}
                                {clicked &&
                                weather.find((e) => e.city === name).opened
                                  ? weather.find((e) => e.city === name)
                                      .tomorrow.parts.day.temp_avg
                                  : ""}{" "}
                              </tspan>
                            </text>
                          </g>
